...
for (def i = 1; i <= findTestData('Scope Roles - Main Grid').getRowNumbers(); i++) {
    ...
    for (def a = 1; a <= findTestData('Details G1').getRowNumbers(); a++) {
        ...
        for (def b = 1; b <= findTestData('Assign Access Groups IT-Roles_G1').getRowNumbers(); b++) {
            WebUI.verifyTextPresent(findTestData('Assign Access Groups IT-Roles_G1').getValue('Role name', b), false, FailureHandling.OPTIONAL)
            ...
            WebUI.verifyTextPresent('Role Name', false, FailureHandling.OPTIONAL)

            WebUI.verifyTextPresent('Role Description', false, FailureHandling.OPTIONAL)

            WebUI.verifyTextPresent('Risk Level', false, FailureHandling.OPTIONAL)

            'verify 4th grid data'
            for (def c = 1; c <= findTestData('Assign Risk Level IT-Roles_G1').getRowNumbers(); c++) {
                WebUI.delay(4)

                WebUI.verifyTextPresent(findTestData('Assign Risk Level IT-Roles_G1').getValue('Role Name', c), false, FailureHandling.OPTIONAL)
